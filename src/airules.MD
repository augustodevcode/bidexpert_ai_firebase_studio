...
**Rule for Seeding Data:** When running the `init-db.ts` or `seed-db.ts` scripts, foreign key constraint checks must be temporarily disabled to avoid insertion order errors. The script should first disable the checks, then run the seed operations, and finally re-enable the checks.

```sql
-- Exemplo de comandos SQL a serem executados no script
SET FOREIGN_KEY_CHECKS = 0;
-- ... (operações de inserção de dados) ...
SET FOREIGN_KEY_CHECKS = 1;
```

**Rationale:** Using Prisma as the schema definition tool ensures type safety and provides a consistent way to manage the database structure. Disabling foreign key checks during seeding is a pragmatic approach in development to allow for flexible and non-sequential data insertion without causing the process to fail.
