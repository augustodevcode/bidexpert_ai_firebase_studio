/**
 * @fileoverview Servi├ºo de Entity Links para MediaItem.
 * Faz reverse-lookup em todas as entidades que referenciam um MediaItem
 * via FK (imageMediaId, logoMediaId, etc.) e retorna informa├º├Áes da entidade vinculada.
 * Quando adicionar novo entity com media FK, atualizar este servi├ºo.
 */
  ...
  try {
    ...
    for (const a of auctions) {
      links.push({
        ...
        entityId: String(a.id),
        publicId: a.publicId ?? null,
        entityName: a.title || `Leil├úo #${a.publicId || a.id}`,
        adminUrl: `/admin/auctions/${a.id}`,
        icon: 'Gavel',
      });
    }

    // Auctioneer ÔåÆ logoMediaId
    ...
    // LotCategory ÔåÆ logoMediaId, coverImageMediaId, megaMenuImageMediaId
    ...
    // DirectSaleOffer ÔåÆ imageMediaId
...
/**
 * Busca entity links para m├║ltiplos MediaItems de uma vez (batch).
 */
export async function getEntityLinksForMediaItems(
  ...
  for (const a of auctions) {
    if (a.imageMediaId) {
      result[String(a.imageMediaId)]?.push({
        entityType: 'Leil├úo', entityId: String(a.id), publicId: a.publicId ?? null,
        entityName: a.title || `Leil├úo #${a.publicId || a.id}`, adminUrl: `/admin/auctions/${a.id}`, icon: 'Gavel',
      });
    }
  }
  for (const a of auctioneers) {
