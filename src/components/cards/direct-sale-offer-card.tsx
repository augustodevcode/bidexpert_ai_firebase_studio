    ...
    <Card data-ai-id={`direct-sale-card-${offer.id}`} className="flex flex-col overflow-hidden h-full shadow-md hover:shadow-lg transition-shadow duration-300 rounded-lg group">
      <div className="relative">
        ...
        <div className="absolute top-2 left-2 flex flex-col items-start gap-1 z-10">
          <Badge className={`text-xs px-2 py-1 ${offer.status === 'ACTIVE' ? 'bg-green-600 text-white' : 'bg-gray-500 text-white'}`}>
      ...
      <CardFooter className="p-3 border-t flex items-end justify-between">
        <div>
            ...
            <p className="text-xl font-bold text-primary">
                ...
                <p className="text-md font-semibold text-primary">Consulte</p>
            )}
        </div>
