...
const AnalyzeTestFailureOutputSchema = z.object({
  analysis: z.string().describe("A brief, insightful analysis of the likely root cause of the test failure."),
...
const analyzeTestFailurePrompt = ai.definePrompt({
    ...
    output: { schema: AnalyzeTestFailureOutputSchema },
    prompt: `
...
const analyzeTestFailureFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const { output } = await analyzeTestFailurePrompt.generate({
      input,
    });
