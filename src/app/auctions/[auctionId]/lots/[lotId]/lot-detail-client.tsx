...
import {
    ...
    ShieldCheck, HelpCircle, ShoppingCart, Heart, X, Facebook, Mail, MessageSquareText, Gavel, ImageOff, Loader2, FileText, ThumbsDown, MessageCircle, Send, Eye, ExternalLink, ListFilter, FileQuestion, Banknote, Building, Link2 as LinkIcon, AlertCircle, Percent, Zap, TrendingUp, Crown, Layers, UserCircle, Scale
} from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-24">
                    <Card className="shadow-md"><CardContent className="p-4 space-y-3"><div className="flex justify-between items-center text-sm"><span className="text-muted-foreground">Visitas: {lot.views}</span><span className="text-muted-foreground">Participantes: {auction.totalHabilitatedUsers || 0}</span><span className="text-muted-foreground">Lances: {lot.bidsCount || 0}</span></div><Separator /><div className="text-sm"><p className="text-muted-foreground">{currentBidLabel}</p><p className="text-3xl font-bold text-primary">R$ {currentBidValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p><p className="text-xs text-muted-foreground">(BRL)</p>{lot.initialPrice && lot.price > lot.initialPrice && (<p className="text-xs text-muted-foreground">Lance Inicial: <span className="line-through">R$ {lot.initialPrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></p>)}{lot.bidIncrementStep && <p className="text-xs text-muted-foreground mt-1">Incremento Mínimo: <span className="font-semibold text-foreground">R$ {lot.bidIncrementStep.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></p>}</div><div className="space-y-2 pt-2">{sectionBadgesLotDetail.showDiscountBadge !== false && mentalTriggersGlobalSettings.showDiscountBadge && discountPercentageLotDetail > 0 && (<Badge variant="destructive" className="text-sm px-2 py-1 w-full justify-center animate-pulse"><Percent className="h-4 w-4 mr-1.5" /> {discountPercentageLotDetail}% DE DESCONTO AGORA!</Badge>)}<div className="flex flex-wrap gap-2 justify-center">{mentalTriggersLotDetail.map(trigger => (<Badge key={trigger} variant="secondary" className="text-xs px-2 py-0.5 bg-amber-100 text-amber-700 border-amber-300">{trigger === 'MAIS VISITADO' && <TrendingUp className="h-3.5 w-3.5 mr-1" />}{trigger === 'LANCE QUENTE' && <Zap className="h-3.5 w-3.5 mr-1 text-red-500 fill-red-500" />}{trigger === 'EXCLUSIVO' && <Crown className="h-3.5 w-3.5 mr-1 text-purple-600" />}{trigger}</Badge>)))}</div>{(lot.allowInstallmentBids || auction.allowInstallmentBids) && (<div className="flex items-center justify-center text-xs text-green-600 bg-green-100 border border-green-300 p-1.5 rounded-md"><Banknote className="h-4 w-4 mr-1.5" />Permite Lance Parcelado (Consulte Condições)</div>)}</div>{canUserBid ? (<div className="space-y-2 pt-2"><div className="relative"><DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input type="number" placeholder={`Próximo lance R$ ${nextMinimumBid.toLocaleString('pt-BR')}`} value={bidAmountInput} onChange={(e) => setBidAmountInput(e.target.value)} className="pl-9 h-11 text-base" min={nextMinimumBid} step={bidIncrement} disabled={isPlacingBid} /><Button size="sm" variant="ghost" className="absolute right-2 top-1/2 -translate-y-1/2 h-7 text-primary" onClick={() => setBidAmountInput(String(nextMinimumBid))}>+</Button></div><p className="text-xs text-muted-foreground text-center">Incremento: R$ {bidIncrement.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</p><Button onClick={handlePlaceBid} disabled={isPlacingBid || !bidAmountInput} className="w-full h-11 text-base bg-accent text-accent-foreground hover:bg-accent/90">{isPlacingBid ? <Loader2 className="animate-spin" /> : bidButtonLabel}</Button><Button variant="link" size="sm" className="w-full text-primary text-xs">Estimar comissões e demais valores</Button><div className="flex items-center space-x-2 justify-center pt-2"><Switch id="quick-bid" disabled /><Label htmlFor="quick-bid" className="text-xs text-muted-foreground">Habilitar lance rápido</Label></div></div>) : (<div className="text-sm text-center p-3 bg-destructive/10 text-destructive rounded-md"><p>{lot.status !== 'ABERTO_PARA_LANCES' ? `Lances para este lote estão ${getAuctionStatusText(lot.status).toLowerCase()}.` : (userProfileWithPermissions ? 'Para ver sua posição na disputa ou dar lances, habilite-se.' : 'Para ver sua posição na disputa, efetue o login.')}</p>{!userProfileWithPermissions && <Link href={`/auth/login?redirect=/auctions/${auction.publicId || auction.id}/lots/${lot.publicId || lot.id}`} className="text-primary hover:underline font-medium">Faça login ou registre-se.</Link>}{userProfileWithPermissions && (!isUserHabilitado && !hasAdminRights && !isEffectivelySuperTestUser) && <Button size="lg" className="w-full mt-3 bg-purple-600 hover:bg-purple-700 text-white">HABILITE-SE</Button>}</div>)}<Button variant="outline" className="w-full" onClick={handleToggleFavorite}><Heart className={`mr-2 h-4 w-4 ${isLotFavorite ? 'fill-red-500 text-red-500' : ''}`} />{isLotFavorite ? 'Remover da Minha Lista' : 'Adicionar à Minha Lista'}</Button></CardContent></Card>
