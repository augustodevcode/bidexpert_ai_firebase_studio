...
import type { Lot, Auction, BidInfo, Review, LotQuestion, SellerProfileInfo, PlatformSettings, AuctionStage, LotCategory, UserLotMaxBid, UserProfileWithPermissions } from '@bidexpert/core';
...
export default function LotDetailClientContent({
  ...
  const checkHabilitationStatus = useCallback(async () => {
    if (userProfileWithPermissions?.uid && auction.id) {
        const status = await checkHabilitationForAuctionAction(userProfileWithPermissions.uid, auction.id);
  ...
  const handleNewReview = async (rating: number, comment: string): Promise<boolean> => {
    if (!userProfileWithPermissions) return false;
    const result = await createReview(lot.publicId || lot.id, userProfileWithPermissions.id, userProfileWithPermissions.fullName || 'Anônimo', rating, comment);
    if(result.success) {
      toast({title: 'Avaliação enviada com sucesso!'});
      const newReviews = await getReviewsForLot(lot.publicId || lot.id);
      setLotReviews(newReviews);
      return true;
    }
    toast({title: 'Erro', description: result.message, variant: 'destructive'});
    return false;
  };
  const handleNewQuestion = async (questionText: string): Promise<boolean> => {
      if (!userProfileWithPermissions) return false;
      const result = await askQuestionOnLot(lot.publicId || lot.id, userProfileWithPermissions.id, userProfileWithPermissions.fullName || 'Usuário', questionText);
       if(result.success) {
        toast({title: 'Pergunta enviada com sucesso!'});
        const newQuestions = await getQuestionsForLot(lot.publicId || lot.id);
        setLotQuestions(newQuestions);
        return true;
      }
      toast({title: 'Erro', description: result.message, variant: 'destructive'});
      return false;
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              ...
              <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-24 self-start">
                  ...
                  />
                  <Card className="shadow-md"><CardHeader><CardTitle className="text-lg font-semibold flex items-center"><Scale className="h-5 w-5 mr-2 text-muted-foreground"/>Valores e Condições Legais</CardTitle></CardHeader><CardContent className="space-y-2 text-sm">{activeLotPrices?.initialBid && <div className="flex justify-between"><span className="text-muted-foreground">Lance Inicial ({activeStage?.name || 'Etapa'}):</span> <span className="font-semibold text-foreground">R$ {activeLotPrices.initialBid.toLocaleString('pt-BR')}</span></div>}{lot.initialPrice && <div className="flex justify-between"><span className="text-muted-foreground">Preço de Reserva:</span> <span className="font-semibold text-foreground">(Confidencial)</span></div>}{lot.evaluationValue && <div className="flex justify-between"><span className="text-muted-foreground">Dívidas Conhecidas:</span> <span className="text-muted-foreground whitespace-pre-line">{lot.knownDebts}</span></div>}{lot.evaluationValue && <div className="flex justify-between"><span className="text-muted-foreground">Outras Informações/Links de Documentos:</span> <span className="text-muted-foreground whitespace-pre-line">{lot.additionalDocumentsInfo}</span></div>}{!activeLotPrices?.initialBid && !lot.evaluationValue && (<p className="text-muted-foreground text-center text-xs py-2">Nenhuma condição de valor especial para este lote.</p>)}</CardContent></Card>
