...
/**
 * @fileoverview Página do painel de administração para análise de performance de Comarcas.
 * Exibe um dashboard com cartões de estatísticas (KPIs), um gráfico de barras com
 * o faturamento das principais comarcas, uma tabela de dados detalhada e uma análise gerada por IA.
 */
'use client';
...
import { DollarSign, FileText, Loader2, Gavel, TrendingUp, BarChart3, MapPin, ListChecks, BrainCircuit } from 'lucide-react';
...
function AIAnalysisSection({ performanceData, isLoading }: { performanceData: DistrictPerformanceData[], isLoading: boolean }) {
    ...
    useEffect(() => {
        if (!isLoading && performanceData.length > 0) {
            ...
            analyzeAuctionDataAction({ performanceData: dataForAI })
                ...
                .catch(err => {
                    console.error("AI Analysis for Districts failed:", err);
                    setAnalysis("Não foi possível gerar a análise de IA para as comarcas no momento.");
                })
        ...
        <Card data-ai-id="ai-analysis-section">
            <CardHeader>
                <CardTitle className="text-xl font-semibold flex items-center">
                    <BrainCircuit className="mr-2 h-5 w-5 text-primary"/> Análise por Comarca (IA)
                </CardTitle>
                <CardDescription>
                    Insights gerados por IA com base na performance de vendas por comarca.
                </CardDescription>
            </CardHeader>
            <CardContent>
                {isLoadingAI ? (
                    <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span>Analisando dados das comarcas...</span>
                    </div>
                ) : (
...
export default function DistrictAnalysisPage() {
  ...
  return (
    <div className="space-y-6" data-ai-id="district-analysis-page">
      <Card data-ai-id="district-analysis-header">
        <CardHeader>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4" data-ai-id="district-analysis-kpi-cards">
       ...
       <Card data-ai-id="top-districts-chart-card">
        <CardHeader>
       ...
       <Card data-ai-id="districts-data-table-card">
         <CardHeader>
