...
export default function LotForm({
  ...
  const form = useForm<LotFormValues>({
    ...
    defaultValues: {
      ...
      debtAmount: initialData?.debtAmount || null,
      itbiValue: initialData?.itbiValue || null,
    },
  });
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <CardContent className="space-y-6">
              ...
              <FormField control={form.control} name="auctionId" render={({ field }) => (<FormItem><FormLabel>Leilão Associado</FormLabel><Select onValueChange={(value) => { field.onChange(value); const selectedAuction = auctions.find(a => a.id === value); form.setValue('auctionName', selectedAuction?.title || ''); }} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Selecione o leilão" /></SelectTrigger></FormControl><SelectContent>{auctions.length === 0 ? <p className="p-2 text-sm text-muted-foreground">Nenhum leilão cadastrado</p> : auctions.map(auction => (<SelectItem key={auction.id} value={auction.id}>{auction.title} (ID: ...{auction.id.slice(-6)})</SelectItem>))}</SelectContent></Select><FormDescription>Associe este lote a um leilão existente.</FormDescription><FormMessage /></FormItem>)} />
              <FormField control={form.control} name="auctionName" render={({ field }) => (<FormItem className="hidden"><FormLabel>Nome do Leilão (Automático)</FormLabel><FormControl><Input {...field} readOnly /></FormControl></FormItem>)} />
              <FormField control={form.control} name="description" render={({ field }) => (<FormItem><FormLabel>Descrição (Opcional)</FormLabel><FormControl><Textarea placeholder="Detalhes sobre o lote..." {...field} value={field.value ?? ""} rows={4} /></FormControl><FormMessage /></FormItem>)} />
              ...
              <div className="grid md:grid-cols-3 gap-6">
               ...
               <Separator />
              <h3 className="text-md font-semibold text-muted-foreground pt-2 flex items-center gap-2"><Scale className="h-5 w-5" /> Valores e Condições Especiais (Opcional)</h3>
              <div className="grid md:grid-cols-2 gap-6">
                <FormField control={form.control} name="evaluationValue" render={({ field }) => (<FormItem><FormLabel>Valor de Avaliação Oficial</FormLabel><FormControl><Input type="number" placeholder="Ex: 100000.00" {...field} value={field.value ?? ''} /></FormControl><FormDescription className="text-xs">Para leilões judiciais, base para cálculo do preço vil.</FormDescription><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="reservePrice" render={({ field }) => (<FormItem><FormLabel>Preço de Reserva Confidencial</FormLabel><FormControl><Input type="number" placeholder="Valor mínimo para venda" {...field} value={field.value ?? ''} /></FormControl><FormDescription className="text-xs">O lote só será vendido se o lance atingir este valor.</FormDescription><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="debtAmount" render={({ field }) => (<FormItem><FormLabel>Montante da Dívida</FormLabel><FormControl><Input type="number" placeholder="Ex: 75000.00" {...field} value={field.value ?? ''} /></FormControl><FormDescription className="text-xs">Usado em leilões de alienação fiduciária.</FormDescription><FormMessage /></FormItem>)} />
