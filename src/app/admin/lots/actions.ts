...
import { sampleLots, sampleAuctions, sampleLotCategories, sampleStates, sampleCities, slugify, sampleBids, sampleLotReviews, sampleLotQuestions, sampleSellers, sampleSubcategories } from '@/lib/sample-data';
import type { Lot, LotFormData, LotDbData, BidInfo, Review, LotQuestion, SellerProfileInfo } from '@/types';
...
export async function createLot(
  ...
  console.log(`[Action - createLot - SampleData Mode] Data received (incl. subcategoryId):`, {
    ...data,
    bidIncrementStep: data.bidIncrementStep,
    subcategoryId: data.subcategoryId, // Log subcategoryId
  });
  await delay(100);
...
export async function getLots(auctionIdParam?: string): Promise<Lot[]> {
  ...
  for (const lot of lotsToReturn) {
    ...
    if (lot.subcategoryId && !lot.subcategoryName) {
        const subcat = sampleSubcategories.find(s => s.id === lot.subcategoryId);
        if (subcat) lot.subcategoryName = subcat.name;
    }
...
export async function getLot(idOrPublicId: string): Promise<Lot | null> {
  ...
  if (lot) {
    ...
    if (enrichedLot.subcategoryId && !enrichedLot.subcategoryName) {
        const subcat = sampleSubcategories.find(s => s.id === enrichedLot.subcategoryId);
        if (subcat) enrichedLot.subcategoryName = subcat.name;
    }
...
export async function updateLot(
  ...
  console.log(`[Action - updateLot - SampleData Mode] Data received for update (incl. subcategoryId):`, {
    ...data,
    bidIncrementStep: data.bidIncrementStep,
    subcategoryId: data.subcategoryId, // Log subcategoryId
  });
