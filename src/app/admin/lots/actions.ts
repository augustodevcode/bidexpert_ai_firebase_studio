...
import { sampleLots, sampleAuctions, sampleLotCategories, sampleStates, sampleCities, slugify, sampleBids, sampleLotReviews, sampleLotQuestions, sampleSellers } from '@/lib/sample-data';
...
export async function getLots(auctionIdParam?: string): Promise<Lot[]> {
  ...
  for (const lot of lotsToReturn) {
    ...
    if (lot.cityId && !lot.cityName) {
        ...
        if (ci) ci.cityName = ci.name; // Corrected to assign to ci.cityName
    }
  }
...
export async function getLot(idOrPublicId: string): Promise<Lot | null> {
  ...
  if (lot) {
    ...
    if (enrichedLot.cityId && !enrichedLot.cityName) {
        ...
        if (ci) enrichedLot.cityName = ci.name; // Corrected to assign to ci.cityName
    }
