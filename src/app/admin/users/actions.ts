...
// Import ensureAdminInitialized conditionally or handle its absence
// For now, it's imported but createUser and deleteUser will guard its usage
...
export async function createUser(
  ...
  console.log(`[createUser Action] ACTIVE_DATABASE_SYSTEM: ${process.env.ACTIVE_DATABASE_SYSTEM}`);
  ...
  if (activeSystem !== 'FIRESTORE' && (!data.password || data.password.length < 6)) {
     return { success: false, message: 'A senha é obrigatória e deve ter pelo menos 6 caracteres para sistemas SQL.' };
  }
  ...
  try {
    if (activeSystem === 'FIRESTORE') {
      ...
      try {
        ...
        if (error.code !== 'auth/user-not-found') throw error; // Re-throw if not 'user-not-found'
      }
      ...
      const userRecord = await localAuthAdmin.createUser({
        ...
        emailVerified: false,
        password: data.password, // Senha é necessária para Firebase Auth
      ...
      userIdToUse = uuidv4(); 
      // Validar senha se for SQL (já feito acima)
      if (!data.password) { // Senha é obrigatória para SQL
        return { success: false, message: 'Senha é obrigatória para sistemas SQL.' };
      }
    }
    ...
    if (data.roleId && data.roleId !== "---NONE---") {
      ...
      if (userRole) {
        ...
        if (userRoleAfterEnsure) {
          ...
          if (activeSystem === 'FIRESTORE') {
            const { auth: localAuthAdminForCleanup } = ensureAdminInitialized();
    ...
    // A senha em texto plano é passada aqui para os adaptadores SQL.
    // No futuro, o hashing deve ocorrer AQUI ANTES de chamar ensureUserProfileInDb se o sistema for SQL.
    const profileResult = await db.ensureUserProfileInDb(
        userIdToUse, 
        ...
        { 
            ...
            passwordClear: data.password // Passando a senha para o adaptador
        }
    );
    ...
    // Adicionar tratamento de erro para violação de constraint UNIQUE no email para SQL
  ...
  try {
    if (activeSystem === 'FIRESTORE') {
      ...
      if (sdkError || !localAuthAdmin) {
        ...
        // Continuar para tentar excluir do DB, mas registrar o aviso
      } else {
...
export async function ensureUserProfileInDb( 
  ...
  additionalProfileData?: Partial<Pick<UserProfileData, 'cpf' | 'cellPhone' | 'dateOfBirth' | 'password' >> // Adicionado password
  ...
  try {
    const db = await getDatabaseAdapter();
    return db.ensureUserProfileInDb(userUid, email || '', fullName, targetRoleName, additionalProfileData);
  } catch (error: any) {
