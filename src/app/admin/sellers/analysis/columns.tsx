...
import type { SellerPerformanceData } from './actions';
import { DataTableColumnHeader } from '@/components/ui/data-table-column-header';
import { Button } from '@/components/ui/button';
import { Eye } from 'lucide-react';
import Link from 'next/link';

export const createColumns = (): ColumnDef<SellerPerformanceData>[] => [
  {
    accessorKey: "name",
    header: ({ column }) => <DataTableColumnHeader column={column} title="Comitente" />,
    cell: ({ row }) => (
      <Link href={`/admin/sellers/${row.original.id}/edit`} className="hover:text-primary font-medium">
        {row.getValue("name")}
      </Link>
    ),
  },
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title="Lotes Cadastrados" />,
    cell: ({ row }) => <div className="text-center">{row.getValue("totalLots")}</div>,
  },
  {
    accessorKey: "totalRevenue",
    header: ({ column }) => <DataTableColumnHeader column={column} title="Faturamento (Pago)" />,
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center justify-end">
            <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                <Link href={`/admin/sellers/${row.original.id}/edit`}>
