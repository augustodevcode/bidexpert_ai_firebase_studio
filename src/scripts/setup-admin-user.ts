...
async function initializeFirebaseAdminSDK() {
  const activeSystem = process.env.ACTIVE_DATABASE_SYSTEM?.toUpperCase();
  if (activeSystem !== 'FIRESTORE') {
    console.log(`[Admin Script] Sistema de DB é '${activeSystem}', pulando inicialização do Firebase Admin SDK para Auth/Firestore.`);
...
async function setupAdminUser() {
  ...
  try {
    const dbAdapter = await getDatabaseAdapter();
    let adminRole = await getRoleByNameFromDb(ADMIN_TARGET_ROLE_NAME);
    if (!adminRole || !adminRole.id) {
      ...
      adminRole = await getRoleByNameFromDb(ADMIN_TARGET_ROLE_NAME);
      if(!adminRole || !adminRole.id){
