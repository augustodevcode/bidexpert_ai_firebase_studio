...
prisma.$use(async (params, next) => {
  const entitiesToGenerateCode = ['Auction', 'Lot', 'Seller', 'User', 'Auctioneer', 'Asset', 'LotCategory', 'Subcategory'];
  ...
  if (params.action === 'create' && entitiesToGenerateCode.includes(params.model!)) {
    ...
    const tenantId = params.args.data.tenantId || '1'; // Default to '1' for landlord or if not present
