...
function getSampleData() {
  ...
  if (!fs.existsSync(dataFilePath)) {
    throw new Error(`[Sample Data Loader] sample-data.local.json not found at ${dataFilePath}`);
  }
  
  const fileContent = fs.readFileSync(dataFilePath, 'utf-8');
  if (!fileContent || fileContent.trim().length < 2) {
    throw new Error("[Sample Data Loader] sample-data.local.json is empty or malformed.");
  }
...
export class SampleDataAdapter implements IDatabaseAdapter {
  ...
  private async _persistData(): Promise<void> {
    ...
    // For this prototype, we'll operate on the in-memory cache.
    // A real implementation would require careful serialization.
    console.log(`[SampleDataAdapter] _persistData called. In this version, data is only modified in-memory.`);
  }
  ...
  async getLotCategories(): Promise<LotCategory[]> {
    ...
    const categories = this.data.sampleLotCategories.map(cat => ({
      ...
      megaMenuImageUrl: this.resolveMediaUrl(cat.megaMenuImageMediaId),
    }));
  ...
  async getLotCategory(idOrSlug: string): Promise<LotCategory | null> {
    ...
    const resolvedCategory = {
      ...category,
      logoUrl: this.resolveMediaUrl(category.logoMediaId),
      coverImageUrl: this.resolveMediaUrl(category.coverImageMediaId),
      megaMenuImageUrl: this.resolveMediaUrl(category.megaMenuImageMediaId),
    };
