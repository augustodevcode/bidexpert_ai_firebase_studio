...
export class PostgresAdapter implements IDatabaseAdapter {
  ...
  async getAuctioneer(idOrPublicId: string): Promise<AuctioneerProfileInfo | null> {
    try {
        const { rows } = await getPool().query('SELECT * FROM auctioneers WHERE id = $1 OR public_id = $2 LIMIT 1', [isNaN(parseInt(idOrPublicId)) ? -1 : parseInt(idOrPublicId), idOrPublicId]);
        if (rows.length === 0) return null;
        return mapToAuctioneerProfileInfo(rows[0]);
    } catch (e: any) {
        console.error(`[PostgresAdapter - getAuctioneer with ID ${idOrPublicId}] Error:`, e);
        return null;
    }
  }
  ...
  async createAuctioneer(data: AuctioneerFormData): Promise<{ success: boolean; message: string; auctioneerId?: string; auctioneerPublicId?: string; }> {
    ...
    try {
        ...
        return { success: true, message: 'Leiloeiro criado com sucesso!', auctioneerId: res.rows[0].id, auctioneerPublicId: publicId };
    } catch (e: any) {
  ...
  async updateAuctioneer(id: string, data: Partial<AuctioneerFormData>): Promise<{ success: boolean; message: string; }> {
    try {
        ...
        const setClauses = Object.keys(updateData).map((key, i) => `"${key.replace(/([A-Z])/g, '_$1').toLowerCase()}" = $${i + 1}`).join(', ');
  ...
  async deleteAuctioneer(id: string): Promise<{ success: boolean; message: string; }> {
    try {
        await getPool().query('DELETE FROM auctioneers WHERE id = $1', [id]);
  ...
  async getSeller(idOrPublicId: string): Promise<SellerProfileInfo | null> {
      try {
          const { rows } = await getPool().query('SELECT * FROM sellers WHERE id = $1 OR public_id = $2 LIMIT 1', [isNaN(parseInt(idOrPublicId)) ? -1 : parseInt(idOrPublicId), idOrPublicId]);
          if (rows.length === 0) return null;
          return mapToSellerProfileInfo(rows[0]);
      } catch (e: any) {
          console.error(`[PostgresAdapter - getSeller with ID ${idOrPublicId}] Error:`, e);
          return null;
      }
  }
  ...
  async createSeller(data: SellerFormData): Promise<{ success: boolean; message: string; sellerId?: string; sellerPublicId?: string; }> {
      ...
      try {
          ...
          return { success: true, message: 'Comitente criado com sucesso!', sellerId: res.rows[0].id, sellerPublicId: publicId };
      } catch (e: any) {
          console.error("[PostgresAdapter - createSeller] Error:", e);
  ...
  async updateSeller(id: string, data: Partial<SellerFormData>): Promise<{ success: boolean; message: string; }> {
      try {
          ...
          const setClauses = Object.keys(updateData).map((key, i) => `"${key.replace(/([A-Z])/g, '_$1').toLowerCase()}" = $${i + 1}`).join(', ');
          const values = Object.values(updateData);
          const query = `UPDATE sellers SET ${setClauses}, updated_at = CURRENT_TIMESTAMP WHERE id = $${values.length + 1}`;
          await getPool().query(query, [...values, id]);
          return { success: true, message: 'Comitente atualizado com sucesso!' };
      } catch (e: any) {
          console.error(`[PostgresAdapter - updateSeller with ID ${id}] Error:`, e);
  ...
  async deleteSeller(id: string): Promise<{ success: boolean; message: string; }> {
      try {
          await getPool().query('DELETE FROM sellers WHERE id = $1', [id]);
          return { success: true, message: 'Comitente excluído com sucesso!' };
      } catch (e: any) {
          console.error(`[PostgresAdapter - deleteSeller with ID ${id}] Error:`, e);
