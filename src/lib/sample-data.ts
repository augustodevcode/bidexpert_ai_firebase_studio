...
export function getSampleStatesAndCities() {
    const states = sampleStates.map((s: Omit<StateInfo, 'id' | 'slug' | 'cityCount'>) => ({
        ...s,
        id: `state-${s.uf.toLowerCase()}`,
        slug: slugify(s.name)
    })) as StateInfo[];

    const cities = sampleCities.map((c: Omit<CityInfo, 'id' | 'slug' | 'stateId' | 'lotCount'>) => {
...
export function getSampleData() {
  ...
  const lots = sampleLots.map(lot => {
    ...
    return {
      ...
      cityName: sampleCities.find(c => c.ibgeCode === lot.cityId)?.name, // Assume cityId is ibgeCode
  ...
  const citiesWithDetails = sampleCities.map(city => ({
      ...city,
      stateName: sampleStates.find(s => s.uf === city.stateUf)?.name,
  }));
