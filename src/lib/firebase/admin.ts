...
export function ensureAdminInitialized(): {
  ...
  if (!adminAppInternalInstance) {
    console.log('[Admin SDK ensureAdminInitialized] No default Firebase app found or needs initialization. Loading service account...');
    ...
    if (credentialsPathFromEnv) {
        ...
        if (fs.existsSync(resolvedEnvPath)) {
            try {
                ...
                console.warn(`[Admin SDK ensureAdminInitialized] Failed to load/parse from GOOGLE_APPLICATION_CREDENTIALS ('${resolvedEnvPath}'): ${e.message}. Trying manual path in project root.`);
            }
        } else {
             console.warn(`[Admin SDK ensureAdminInitialized] GOOGLE_APPLICATION_CREDENTIALS path does not exist: ${resolvedEnvPath}. Trying manual path in project root.`);
        }
    } else {
      console.log(`[Admin SDK ensureAdminInitialized] GOOGLE_APPLICATION_CREDENTIALS is NOT set. Trying manual path in project root: ${serviceAccountKeyFileName}`);
    }

    if (!serviceAccount) {
      // Use process.cwd() to build the path from the project root
      const manualPath = path.join(currentCwd, serviceAccountKeyFileName);
      console.log(`[Admin SDK ensureAdminInitialized] Attempting manual path from project root: ${manualPath}`);
