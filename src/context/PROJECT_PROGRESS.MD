...
- **Item 27: Refatorar Testes de Integração e E2E:**
    ...
    - **Status Atual:** Testes de `bidding-e2e`, `wizard-e2e`, `user`, `seller`, `auctioneer`, `auction`, `lot`, `judicial-process`, `state`, `court`, `city`, `judicial-branch`, `judicial-district`, `bem`, `category`, `subcategory`, e `role` refatorados. Próximo passo é continuar com os demais arquivos de teste.

## NEXT
- **Item 28: Concluir Refatoração dos Testes:** Aplicar o padrão de chamada de `Server Actions` a todos os testes de entidade restantes (ex: `direct-sale-offer.test.ts`, `media.test.ts`, etc.).
