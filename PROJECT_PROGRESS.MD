...
## DOING
- **(Em andamento) Item 30: Implementar Herança de Mídia:** Implementar a lógica de herança e substituição de imagens para Lotes e Leilões, conforme definido nas regras de negócio. (Progresso: 60%)
- **(Em andamento) Item 35: Criar Testes de UI por Cenários Específicos:** Criar testes na pasta `tests/ui-e2e` para validar fluxos de usuário complexos, como o loteamento de múltiplos bens, herança de mídia e criação de leilões com múltiplas praças. (Progresso: 60%)
