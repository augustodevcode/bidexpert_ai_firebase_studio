...
## NEXT
- **Item 28: Documentar Arquivos de Código-Fonte:** Adicionar um bloco de comentário explicativo (docblock) no topo de cada arquivo de código-fonte (`.ts`, `.tsx`, etc.) descrevendo seu propósito e responsabilidades.
...
- **Item 37: Implementar Dashboard Customizável:**
    - **37.1:** Adicionar um botão de engrenagem ao `AdminHeader` para abrir o modal de configuração.
    - **37.2:** Criar o componente de modal `WidgetConfigurationModal.tsx`.
    - **37.3:** Listar os widgets disponíveis com checkboxes no modal de configuração.
    - **37.4:** Implementar a lógica para salvar e carregar as preferências de widgets do usuário usando React Context e `localStorage`.
    - **37.5:** Fazer com que a página `/admin/dashboard` renderize os widgets dinamicamente com base nas preferências salvas.

## NEXT
- **Item 38.1:** Criar o Modelo `Subscriber` no Prisma.
