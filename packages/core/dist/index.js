...
__export(index_exports, {
  ...
  AuctioneerProfileInfoSchema: () => AuctioneerProfileInfoSchema,
  ...
  SellerProfileInfoSchema: () => SellerProfileInfoSchema,
  SellerService: () => SellerService,
  UserProfileDataSchema: () => UserProfileDataSchema,
  UserWinService: () => UserWinService,
  accountTypeValues: () => accountTypeValues,
  auctionMethodValues: () => auctionMethodValues,
  auctionParticipationValues: () => auctionParticipationValues,
  auctionStatusValues: () => auctionStatusValues,
  auctionTypeValues: () => auctionTypeValues,
  checkoutFormSchema: () => checkoutFormSchema,
  lotStatusValues: () => lotStatusValues,
  paymentStatusValues: () => paymentStatusValues,
  slugify: () => slugify,
  userHabilitationStatusValues: () => userHabilitationStatusValues
});
...
var BiddingSettingsSchema = import_zod.z.object({
  instantBiddingEnabled: import_zod.z.boolean().default(true),
  getBidInfoInstantly: import_zod.z.boolean().default(true),
  biddingInfoCheckIntervalSeconds: import_zod.z.number().int().min(1).max(60).default(1)
});
var AuctionStageSchema = import_zod.z.object({
  name: import_zod.z.string().min(1),
  endDate: import_zod.z.union([import_zod.z.date(), import_zod.z.string().datetime()]),
  initialPrice: import_zod.z.number().positive().optional(),
  statusText: import_zod.z.string().optional()
});
var AuctionSchema = import_zod.z.object({
  id: import_zod.z.string(),
  publicId: import_zod.z.string(),
  title: import_zod.z.string().min(5),
  description: import_zod.z.string().optional(),
  status: import_zod.z.enum(auctionStatusValues).default("RASCUNHO"),
  auctionDate: import_zod.z.union([import_zod.z.date(), import_zod.z.string().datetime()]),
  endDate: import_zod.z.union([import_zod.z.date(), import_zod.z.string().datetime()]).optional().nullable(),
  totalLots: import_zod.z.number().int().optional(),
  categoryId: import_zod.z.string().optional(),
  auctioneerId: import_zod.z.string(),
  sellerId: import_zod.z.string(),
  imageUrl: import_zod.z.string().url().optional().nullable(),
  imageMediaId: import_zod.z.string().optional().nullable(),
  visits: import_zod.z.number().int().optional(),
  auctionType: import_zod.z.enum(["JUDICIAL", "EXTRAJUDICIAL", "PARTICULAR", "TOMADA_DE_PRECOS", "DUTCH", "SILENT"]).optional(),
  auctionStages: import_zod.z.array(AuctionStageSchema).optional(),
  biddingSettings: BiddingSettingsSchema.optional(),
  createdAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()]),
  updatedAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()])
  // Add other fields as needed, matching the types/index.ts Auction interface
});
var LotSchema = import_zod.z.object({
  id: import_zod.z.string(),
  publicId: import_zod.z.string(),
  auctionId: import_zod.z.string(),
  number: import_zod.z.string().optional(),
  title: import_zod.z.string().min(5),
  description: import_zod.z.string().optional().nullable(),
  price: import_zod.z.number(),
  initialPrice: import_zod.z.number().optional().nullable(),
  status: import_zod.z.enum(lotStatusValues).default("EM_BREVE"),
  bidsCount: import_zod.z.number().int().optional(),
  views: import_zod.z.number().int().optional(),
  imageUrl: import_zod.z.string().url().optional().nullable(),
  imageMediaId: import_zod.z.string().optional().nullable(),
  categoryId: import_zod.z.string().optional(),
  bemIds: import_zod.z.array(import_zod.z.string()).optional(),
  createdAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()]),
  updatedAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()])
  // Add other fields as needed
});
var UserProfileDataSchema = import_zod.z.object({
  id: import_zod.z.string(),
  uid: import_zod.z.string(),
  email: import_zod.z.string().email(),
  password: import_zod.z.string().optional(),
  fullName: import_zod.z.string().nullable(),
  habilitationStatus: import_zod.z.enum(userHabilitationStatusValues),
  accountType: import_zod.z.enum(accountTypeValues),
  roleIds: import_zod.z.array(import_zod.z.string()),
  createdAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()]),
  updatedAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()])
  // Add other fields from UserProfileData interface
});
var SellerProfileInfoSchema = import_zod.z.object({
  id: import_zod.z.string(),
  publicId: import_zod.z.string(),
  slug: import_zod.z.string(),
  name: import_zod.z.string(),
  isJudicial: import_zod.z.boolean().default(false),
  createdAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()]),
  updatedAt: import_zod.z.union([import_zod.z.date(), import_zod.z.string()])
  // Add other fields
});
var AuctioneerProfileInfoSchema = import_zod.z.object({
  id: import_zod.z.string(),
  publicId: import_zod.z.string(),
  slug: import_zod.z.string(),
  name: import_zod.z.string(),
...
var cardSchema = import_zod.z.object({
  cardholderName: import_zod.z.string().min(3, { message: "O nome no cart\xE3o \xE9 obrigat\xF3rio." }),
  cardNumber: import_zod.z.string().min(16, { message: "O n\xFAmero do cart\xE3o deve ter 16 d\xEDgitos." }).max(16, { message: "O n\xFAmero do cart\xE3o deve ter 16 d\xEDgitos." }).regex(/^\d+$/, { message: "O n\xFAmero do cart\xE3o deve conter apenas d\xEDgitos." }),
  expiryDate: import_zod.z.string().regex(/^(0[1-9]|1[0-2])\/\d{2}$/, { message: "Formato de validade inv\xE1lido. Use MM/AA." }).refine((val) => {
  ...
  cvc: import_zod.z.string().min(3, { message: "CVC deve ter 3 d\xEDgitos." }).max(4, { message: "CVC inv\xE1lido." })
});
var checkoutFormSchema = import_zod.z.object({
  paymentMethod: import_zod.z.enum(["credit_card", "installments"], {
    required_error: "Selecione um m\xE9todo de pagamento."
  }),
  installments: import_zod.z.coerce.number().optional(),
  ...
  async createLot(data) {
    try {
      ...
      const dataToCreate = {
        ...
        publicId: `LOTE-PUB-${(0, import_uuid2.v4)().substring(0, 8)}`,
...
var SellerService = class {
  ...
  async createSeller(data) {
    try {
      ...
      const dataToCreate = {
        ...
        publicId: `COM-${(0, import_uuid3.v4)()}`
      };
      if (userId) {
  ...
  async getSellerDashboardData(sellerId) {
    ...
    const now = /* @__PURE__ */ new Date();
    for (let i = 11; i >= 0; i--) {
      const date = (0, import_date_fns.subMonths)(now, i);
      const monthKey = (0, import_date_fns.format)(date, "MMM/yy", { locale: import_locale.ptBR });
    ...
    paidWins.forEach((win) => {
      ...
      const monthKey = (0, import_date_fns.format)(winDate, "MMM/yy", { locale: import_locale.ptBR });
...
var AuctioneerService = class {
  ...
  async criarLeiloeiro(data) {
    try {
      const dataToCreate = {
        ...data,
        slug: slugify(data.name),
        publicId: `LEILOE-${(0, import_uuid4.v4)()}`
      };
  ...
  async obterDadosDashboardLeiloeiro(auctioneerId) {
    ...
    const now = /* @__PURE__ */ new Date();
    for (let i = 11; i >= 0; i--) {
      const date = (0, import_date_fns2.subMonths)(now, i);
      const monthKey = (0, import_date_fns2.format)(date, "MMM/yy", { locale: import_locale2.ptBR });
      salesByMonthMap.set(monthKey, 0);
    }
    allLotsFromAuctions.forEach((lot) => {
      if (lot.updatedAt) {
        const monthKey = (0, import_date_fns2.format)(new Date(lot.updatedAt), "MMM/yy", { locale: import_locale2.ptBR });
        if (salesByMonthMap.has(monthKey)) {
...
var HabilitationService = class {
  ...
  async habilitateForAuction(userId, auctionId) {
    try {
      ...
      if (process.env.NODE_ENV !== "test") {
        (0, import_cache.revalidatePath)(`/auctions/${auctionId}`);
      }
  ...
  async saveUserDocument(userId, documentTypeId, fileUrl, fileName) {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "test") {
        (0, import_cache.revalidatePath)("/dashboard/documents");
        (0, import_cache.revalidatePath)(`/admin/habilitations/${userId}`);
      }
  ...
  async approveDocument(documentId, analystId) {
    try {
      ...
      if (process.env.NODE_ENV !== "test") {
        (0, import_cache.revalidatePath)("/admin/habilitations");
        (0, import_cache.revalidatePath)(`/admin/habilitations/${docToUpdate.userId}`);
      }
...
var BidService = class {
  ...
  async placeBid(lotIdOrPublicId, auctionIdOrPublicId, userId, userDisplayName, bidAmount) {
    try {
      ...
      if (process.env.NODE_ENV !== "test") {
        (0, import_cache2.revalidatePath)(`/auctions/${auctionIdOrPublicId}/lots/${lotIdOrPublicId}`);
        (0, import_cache2.revalidatePath)(`/auctions/${auctionIdOrPublicId}/live`);
        (0, import_cache2.revalidatePath)(`/live-dashboard`);
      }
  ...
  async placeMaxBid(lotId, userId, maxAmount) {
    try {
      ...
      if (process.env.NODE_ENV !== "test") {
        (0, import_cache2.revalidatePath)(`/auctions/${lot.auctionId}/lots/${lot.publicId || lot.id}`);
      }
