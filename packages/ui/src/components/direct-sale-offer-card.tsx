// packages/ui/src/components/direct-sale-offer-card.tsx
'use client';
...
export function DirectSaleOfferCard({ offer }: DirectSaleOfferCardProps) {
  ...
  return (
    <Card className="flex flex-col overflow-hidden h-full shadow-md hover:shadow-lg transition-shadow duration-300 rounded-lg group" data-ai-id={`direct-sale-card-container-${offer.id}`}>
      ...
      <CardContent className="p-3 flex-grow space-y-1.5">
        ...
        <Link href={`/direct-sales/${offer.id}`}>
          <h3 className="text-sm font-semibold hover:text-primary transition-colors leading-tight min-h-[2.2em] line-clamp-2" data-ai-id={`direct-sale-card-title-${offer.id}`}>
            {offer.title}
          </h3>
        </Link>
