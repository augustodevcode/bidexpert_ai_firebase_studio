// packages/ui/src/components/lot-preview-modal.tsx
'use client';

import type { Lot, Auction, PlatformSettings, AuctionStage, LotCategory, UserLotMaxBid } from '@bidexpert/core';
...
import { isPast, differenceInSeconds, format, isValid } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { getAuctionStatusText, getLotStatusColor, getEffectiveLotEndDate, slugify, getActiveStage, getLotPriceForStage } from '../lib/ui-helpers';
...
const TimeRemaining: React.FC<{endDate: Date | null}> = ({ endDate }) => {
...
export default function LotPreviewModal({ lot, auction, platformSettings, isOpen, onClose }: LotPreviewModalProps) {
  ...
  const formattedEndDate = effectiveLotEndDate ? format(effectiveLotEndDate, "dd/MM/yyyy 'às' HH:mm", { locale: ptBR }) : 'Não definida';
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    <div className="flex items-center text-destructive font-semibold">
                        ...
                        <span className="text-lg">Encerra em: <TimeRemaining endDate={effectiveLotEndDate} /></span>
                    </div>
