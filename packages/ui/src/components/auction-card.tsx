...
export function AuctionCard({ auction, onUpdate }: AuctionCardProps) {
        ...
        <Card data-ai-id={`auction-card-container-${auction.id}`} className="flex flex-col overflow-hidden h-full shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg group">
          <div className="relative">
            <Link href={`/auctions/${auction.publicId || auction.id}`} className="block">
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Link href={sellerSlug ? `/sellers/${sellerSlug}` : '#'} onClick={(e) => e.stopPropagation()} className="absolute bottom-2 right-2 z-10">
                                <Avatar className="h-12 w-12 border-2 bg-background border-border shadow-md" data-ai-id={`auction-card-seller-logo-${auction.id}`}>
             ...
             <div className="absolute top-2 left-2 flex flex-col items-start gap-1 z-10" data-ai-id={`auction-card-badges-${auction.id}`}>
                <Badge className={`text-xs px-2 py-1 ${statusDisplay.className}`}>
                    {statusDisplay.text}
                </Badge>
            </div>
             <div className="absolute top-2 right-2 flex flex-col items-end gap-1 z-10" data-ai-id={`auction-card-mental-triggers-${auction.id}`}>
          ...
          <CardContent className="p-4 flex-grow">
            <div className="flex justify-between items-start text-xs text-muted-foreground mb-1">
              <span className="truncate" title={`ID: ${auction.publicId || auction.id}`} data-ai-id={`auction-card-public-id-${auction.id}`}>ID: {auction.publicId || auction.id}</span>
              {auctionTypeDisplay && (
                <div className="flex items-center gap-1" data-ai-id={`auction-card-type-${auction.id}`}>
            ...
            <Link href={`/auctions/${auction.publicId || auction.id}`}>
              <h3 data-ai-id={`auction-card-title-${auction.id}`} className="text-md font-semibold hover:text-primary transition-colors mb-2 leading-tight min-h-[2.5em] line-clamp-2">
                {auction.title}
              </h3>
            </Link>
            
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs text-muted-foreground mb-2" data-ai-id={`auction-card-counters-${auction.id}`}>
          ...
          <CardFooter className="p-4 border-t flex-col items-start space-y-2">
            {auction.initialOffer && (
              <div className="w-full" data-ai-id={`auction-card-initial-offer-${auction.id}`}>
