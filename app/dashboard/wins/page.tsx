...
import { generateWinningBidTermAction } from '@/app/admin/lots/actions';
...
export default function MyWinsPage() {
  ...
  const [isGeneratingDoc, setIsGeneratingDoc] = useState<Record<string, boolean>>({});
  ...
  const handleGenerateTerm = async (lotId: string) => {
      setIsGeneratingDoc(prev => ({...prev, [lotId]: true }));
      toast({ title: 'Gerando Documento...', description: 'Aguarde, o Auto de Arrematação está sendo criado.'});
      try {
          ...
          if (result.success && result.pdfBase64) {
             const byteCharacters = atob(result.pdfBase64);
              ...
              link.download = result.fileName || `termo-arrematacao-${lotId}.pdf`;
              link.click();
              toast({ title: 'Sucesso!', description: 'Download do seu documento foi iniciado.'});
              await fetchWins(userProfileWithPermissions!.uid); // Refresh data to get the new URL
          } else {
         ...
         toast({ title: 'Erro ao Gerar Documento', description: error.message, variant: 'destructive'});
          ...
          setIsGeneratingDoc(prev => ({...prev, [lotId]: false }));
      }
  };
                ...
                return (
                  <Card key={win.id} className="overflow-hidden shadow-md flex flex-col">
                    ...
                    <CardFooter className="border-t pt-4 flex flex-col sm:flex-row flex-wrap gap-2">
                       {win.lot.winningBidTermUrl ? (
                         <Button variant="secondary" size="sm" className="flex-1 min-w-[calc(50%-0.25rem)]" asChild>
                            <a href={win.lot.winningBidTermUrl} target="_blank" rel="noopener noreferrer">
                                <FileText className="mr-2 h-4 w-4" /> Ver Auto de Arrematação
                            </a>
                         </Button>
                        ) : (
                         <Button variant="secondary" size="sm" className="flex-1 min-w-[calc(50%-0.25rem)]" onClick={() => handleGenerateTerm(win.lotId)} disabled={isGeneratingDoc[win.lotId]}>
                            {isGeneratingDoc[win.lotId] ? <Loader2 className="animate-spin mr-2 h-4 w-4"/> : <FileText className="mr-2 h-4 w-4" />}
