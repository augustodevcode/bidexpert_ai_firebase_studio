 ...
 * @param {string | null} tenantId - O ID do tenant no qual o usuário está logando. Se for nulo, tenta usar o padrão do .env.
 */
export async function createSession(user: UserProfileWithPermissions, tenantId: string | null) {
    const finalTenantId = tenantId || process.env.DEFAULT_TENANT_ID;

    if (!finalTenantId) {
        throw new Error('Não foi possível determinar o Tenant ID para a sessão.');
    }
    
    console.log(`[Create Session] Criando sessão para usuário ${user.email} no tenant ${finalTenantId}`);
    
    const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    const sessionPayload = {
        userId: user.id,
        email: user.email,
        tenantId: finalTenantId, // Adiciona o tenantId ao payload da sessão
