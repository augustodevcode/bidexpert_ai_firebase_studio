...
export default function ResourceDataTable<TData extends { id: string, name?: string | null, title?: string | null }>({
  columns,
  fetchAction,
  deleteAction,
  searchColumnId,
  ...
  const handleDeleteSelected = useCallback(async (selectedItems: TData[]) => {
    ...
    for (const item of selectedItems) {
       if (deleteConfirmation && !deleteConfirmation(item)) {
        toast({ title: `Ação não Permitida`, description: deleteConfirmationMessage ? deleteConfirmationMessage(item) : `O item "${item.name || item.title || item.id}" não pode ser excluído.`, variant: "destructive", duration: 5000 });
        errorCount++;
        continue;
      }
      ...
      if (result.success) {
        ...
        errorCount++;
        toast({ title: `Erro ao excluir ${item.name || item.title || item.id}`, description: result.message, variant: "destructive", duration: 5000 });
      }
    }
  ...
  return (
    <div data-ai-id={`resource-data-table-${searchColumnId}`}>
        <DataTable
