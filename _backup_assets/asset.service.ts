...
export class AssetService {
  ...
  async createAsset(tenantId: string, data: AssetFormData): Promise<{ success: boolean; message: string; assetId?: string; }> {
    try {
      const { categoryId, subcategoryId, judicialProcessId, sellerId, cityId, stateId, properties, ...assetData } = data;

      const normalizedAssetData = { ...assetData } as Record<string, any>;
      if (typeof normalizedAssetData.imageMediaId === 'string' && normalizedAssetData.imageMediaId.trim() === '') {
        normalizedAssetData.imageMediaId = null;
      }
      if (typeof normalizedAssetData.imageUrl === 'string' && normalizedAssetData.imageUrl.trim() === '') {
        normalizedAssetData.imageUrl = null;
      }
      ...
      const dataToCreate: Prisma.AssetCreateInput = {
        ...(normalizedAssetData as any),
        publicId,
        tenant: { connect: { id: BigInt(tenantId) } },
      };

      // Campo "properties" não está disponível no schema atual do Prisma para Asset;
      // ignoramos o valor recebido para evitar erros de argumento desconhecido.
  ...
  async updateAsset(id: string, data: Partial<AssetFormData>): Promise<{ success: boolean; message: string; }> {
    try {
      ...
      const dataToUpdate: Prisma.AssetUpdateInput = { ...(normalizedAssetData as any) };
      // Mesmo racional do createAsset: não enviamos "properties" para o Prisma.
