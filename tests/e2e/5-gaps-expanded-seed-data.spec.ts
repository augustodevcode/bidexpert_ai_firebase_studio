...
test.describe('Expanded Seed Data: Processos Judiciais', () => {
  ...
  test('PJ5: Deve exibir detalhes completos dos processos', async ({ page }) => {
    // Clicar no primeiro processo para ver detalhes
    const processLink = page.locator('a:has-text(/\\d{7}-\\d{2}/'), [data-testid*="process-link"]').first();
    
    if (await processLink.count() > 0) {
      await processLink.click();
      await page.waitForURL(/\/judicial-processes\/\d+|\/process.*details/i, { timeout: 5000 }).catch(() => {
        console.log('⚠️ Navegação para detalhes pode ter falhado');
      });

      // Verificar que aparecem campos de detalhes
      const detailsContainer = page.locator('[data-testid="process-details"], .process-details-container').first();
  ...
  test('PJ6: Deve permitir filtrar processos por comarca', async ({ page }) => {
    // Procurar select de comarca
    const comarcaSelect = page.locator('select[name*="comarca" i], [data-testid*="comarca-filter"]').first();
    
    if (await comarcaSelect.count() > 0) {
      // Selecionar Rio de Janeiro
      await comarcaSelect.selectOption(/Rio de Janeiro|RJ/);
      await page.waitForTimeout(500);
      ...
      if (await rjElement.count() > 0) {
        await expect(rjElement).toBeVisible();
        console.log('✓ Filtro de comarca funciona');
      }
    }
  });
});
...
// ═══════════════════════════════════════════════════════════════════════════
// TESTES: VENDEDORES JUDICIAIS POR REGIÃO
// ═══════════════════════════════════════════════════════════════════════════

test.describe('Expanded Seed Data: Vendedores Judiciais', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${BASE_URL}/admin/judicial-sellers`, { waitUntil: 'networkidle' }).catch(() => {
      return page.goto(`${BASE_URL}/admin/sellers`, { waitUntil: 'networkidle' });
    });
  });

  test('VJ1: Deve exibir 3 vendedores judiciais', async ({ page }) => {
    // Aguardar tabela
    const table = page.locator('table, [data-testid="sellers-list"], .sellers-grid').first();
    
    if (await table.count() > 0) {
      await table.waitFor({ state: 'visible', timeout: 5000 });

      const vendedores = ['Leiloeiro Judicial SP', 'Leiloeiro Judicial RJ', 'Leiloeiro Judicial MG'];
      
      for (const vendedor of vendedores) {
        const element = page.locator(`text=${vendedor}`).first();
        if (await element.count() > 0) {
          console.log(`✓ Encontrado: ${vendedor}`);
        }
      }
    }
  });

  test('VJ2: Deve exibir vendedor vinculado a São Paulo', async ({ page }) => {
    const spVendedor = page.locator('text=Leiloeiro Judicial SP').first();
    
    if (await spVendedor.count() > 0) {
      await expect(spVendedor).toBeVisible();
      
      // Verificar que está vinculado a SP
      const spRegion = spVendedor.locator('text=SP').first();
      if (await spRegion.count() > 0) {
        console.log('✓ Vendedor SP vinculado corretamente');
      }
    }
  });

  test('VJ3: Deve exibir vendedor vinculado a Rio de Janeiro', async ({ page }) => {
    const rjVendedor = page.locator('text=Leiloeiro Judicial RJ').first();
    
    if (await rjVendedor.count() > 0) {
      await expect(rjVendedor).toBeVisible();
      console.log('✓ Vendedor RJ encontrado');
    }
  });

  test('VJ4: Deve exibir vendedor vinculado a Minas Gerais', async ({ page }) => {
    const mgVendedor = page.locator('text=Leiloeiro Judicial MG').first();
    
    if (await mgVendedor.count() > 0) {
      await expect(mgVendedor).toBeVisible();
      console.log('✓ Vendedor MG encontrado');
    }
  });
});

// ═══════════════════════════════════════════════════════════════════════════
// TESTES: INTEGRAÇÃO - DADOS EXPANDIDOS FUNCIONAM JUNTOS
// ═══════════════════════════════════════════════════════════════════════════

test.describe('Expanded Seed Data: Integração', () => {
  test('INT-E1: Pode navegar de auction para lotes', async ({ page }) => {
    // Ir para auctions
    await page.goto(`${BASE_URL}/admin/auctions`, { waitUntil: 'networkidle' });

    // Clicar em uma auction RJ
    const rjAuction = page.locator('text=Leilão Judicial - Imóveis RJ, a[href*="auctions"], [data-testid*="auction-link"]').first();
    
    if (await rjAuction.count() > 0) {
      await rjAuction.click();
      await page.waitForURL(/\/auctions\/\d+/, { timeout: 5000 }).catch(() => {
        console.log('⚠️ Navegação pode ter falhado');
      });

      // Verificar que aparecem lotes
      const lotsSection = page.locator('[data-testid="lots-section"], .auction-lots, h2:has-text("Lotes")').first();
      if (await lotsSection.count() > 0) {
        console.log('✓ Lotes aparecem na auction RJ');
      }
    }
  });

  test('INT-E2: Pode navegar de lote para auction', async ({ page }) => {
    // Ir para lotes
    await page.goto(`${BASE_URL}/admin/lots`, { waitUntil: 'networkidle' });

    // Clicar em um lote
    const lotLink = page.locator('a:has-text(/L\\d+/), [data-testid="lot-link"]').first();
    
    if (await lotLink.count() > 0) {
      await lotLink.click();
      await page.waitForURL(/\/lots\/\d+/, { timeout: 5000 }).catch(() => {
        console.log('⚠️ Navegação para detalhes do lote pode ter falhado');
      });
