...
async function createSearchTestData() {
    ...
    const [stateSP, stateRJ] = await prismaClient.$transaction([
        prismaClient.state.create({ data: { name: 'São Paulo', uf: `S${testRunId.slice(0,1)}`, slug: `sp-${testRunId}` } }),
        prismaClient.state.create({ data: { name: 'Rio de Janeiro', uf: `R${testRunId.slice(0,1)}`, slug: `rj-${testRunId}` } })
    ]);

    const [citySP, cityRJ] = await prismaClient.$transaction([
        prismaClient.city.create({ data: { name: 'São Paulo', slug: `sao-paulo-${testRunId}`, stateId: stateSP.id } }),
        prismaClient.city.create({ data: { name: 'Rio de Janeiro', slug: `rio-de-janeiro-${testRunId}`, stateId: stateRJ.id } })
    ]);
    ...
    const [auc1, auc2, auc3] = await prismaClient.$transaction([
        prismaClient.auction.create({ data: { title: `Leilão de Carros SP ${testRunId}`, slug: `leilao-carros-sp-${testRunId}`, publicId: `pub-auc-1-${testRunId}`, status: 'ABERTO_PARA_LANCES', auctionDate: now, auctioneerId: auctioneer1.id, sellerId: seller1.id, categoryId: category1.id, cityId: citySP.id, stateId: stateSP.id, latitude: -23.550520, longitude: -46.633308, tenantId: testTenant.id } as any }),
        prismaClient.auction.create({ data: { title: `Leilão de Apartamentos RJ ${testRunId}`, slug: `leilao-apartamentos-rj-${testRunId}`, publicId: `pub-auc-2-${testRunId}`, status: 'EM_BREVE', auctionDate: new Date(Date.now() + 86400000), auctioneerId: auctioneer1.id, sellerId: seller2.id, categoryId: category2.id, cityId: cityRJ.id, stateId: stateRJ.id, latitude: -22.906847, longitude: -43.172896, tenantId: testTenant.id } as any }),
        prismaClient.auction.create({ data: { title: `Leilão Misto SP ${testRunId}`, slug: `leilao-misto-sp-${testRunId}`, publicId: `pub-auc-3-${testRunId}`, status: 'ABERTO_PARA_LANCES', auctionDate: now, auctioneerId: auctioneer1.id, sellerId: seller1.id, categoryId: category1.id, cityId: citySP.id, stateId: stateSP.id, latitude: -23.5613, longitude: -46.6800, tenantId: testTenant.id } as any })
    ]);
    ...
    const [l1, l2, l3] = await prismaClient.$transaction([
        prismaClient.lot.create({ data: { title: `Ford Ka 2019 ${testRunId}`, publicId: `pub-lot-1-${testRunId}`, slug: `ford-ka-2019-${testRunId}`, auctionId: auction1.id, price: 35000, type: category1.name, categoryId: category1.id, status: 'ABERTO_PARA_LANCES', cityName: 'São Paulo', stateUf: 'SP', latitude: -23.550520, longitude: -46.633308, tenantId: testTenant.id } }),
        prismaClient.lot.create({ data: { title: `Apartamento 2 Quartos ${testRunId}`, publicId: `pub-lot-2-${testRunId}`, slug: `apartamento-2-quartos-${testRunId}`, auctionId: auction2.id, price: 250000, type: category2.name, categoryId: category2.id, status: 'EM_BREVE', cityName: 'Rio de Janeiro', stateUf: 'RJ', latitude: -22.906847, longitude: -43.172896, tenantId: testTenant.id } }),
        prismaClient.lot.create({ data: { title: `Ford Maverick Antigo ${testRunId}`, publicId: `pub-lot-3-${testRunId}`, slug: `ford-maverick-antigo-${testRunId}`, auctionId: auction3.id, price: 95000, type: category1.name, categoryId: category1.id, status: 'ABERTO_PARA_LANCES', cityName: 'São Paulo', stateUf: 'SP', latitude: -23.5613, longitude: -46.6800, tenantId: testTenant.id } })
    ]);
    lot1 = l1 as any;
    lot2 = l2 as any;
    lot3 = l3 as any;
...
async function cleanupSearchTestData() {
  ...
  try {
    ...
    await prismaClient.city.deleteMany({ where: { slug: { contains: testRunId } } });
    await prismaClient.state.deleteMany({ where: { slug: { contains: testRunId } } });
...
test.describe('Módulo 5: Funcionalidades Públicas - Busca e Filtro (UI)', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: 'Resultados da Busca' })).toBeVisible({ timeout: 20000 });
    });
