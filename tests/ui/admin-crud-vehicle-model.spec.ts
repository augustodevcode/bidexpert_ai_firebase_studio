...
test.describe('Módulo 1: Administração - CRUD de Modelo de Veículo (UI)', () => {
  ...
  test('Cenário: should perform a full CRUD cycle for a Vehicle Model', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Gerenciar Modelos de Veículos' })).toBeVisible({ timeout: 20000 });
    ...
    await expect(page.getByText('Modelo criado com sucesso.')).toBeVisible({ timeout: 10000 });
    ...
    // --- DELETE ---
    const rowToDelete = page.getByRole('row', { name: new RegExp(updatedModelName, 'i') });
