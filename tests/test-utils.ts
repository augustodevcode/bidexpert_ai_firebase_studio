...
import type { UserProfileWithPermissions, Role, SellerProfileInfo, AuctioneerProfileInfo, LotCategory, Auction, Lot, Bem, JudicialProcess, StateInfo, JudicialDistrict, Court, JudicialBranch, Tenant } from '@/types';
...
export async function createTestPrerequisites(testRunId: string, prefix: string) {
    ...
    const adminUser = await callActionAsUser(createUser, null, {
    ...
    }).then(res => callActionAsUser(getUserProfileData, null, res.userId!));
    ...
    const auctioneer = await runInTenant(tenant.id, () => prisma.auctioneer.create({ data: { name: `Auctioneer ${prefix} ${testRunId}`, slug: `auct-${prefix}-${testRunId}`, publicId: `auct-pub-${prefix}-${testRunId}`, tenantId: tenant.id } }));
    
    const uniqueUf = `${prefix.substring(0,1).toUpperCase()}${testRunId.substring(0, 1).toUpperCase()}`;
    const state = await prisma.state.create({ data: { name: `State ${prefix} ${testRunId}`, uf: uniqueUf, slug: `st-${prefix}-${testRunId}` } });
    ...
    const bemRes = await callActionAsUser(createBem, adminUser, { title: `Bem para ${prefix} ${testRunId}`, judicialProcessId: judicialProcess.id, categoryId: category.id, status: 'DISPONIVEL', evaluationValue: 50000.00 } as any);
    const bem = (await prisma.bem.findUnique({where: {id: bemRes.bemId}}))!;

    return { tenant, adminUser, unauthorizedUser, category, auctioneer, judicialSeller, state, court, district, branch, judicialProcess, bem };
}
...
export async function cleanup(testRunId: string, prefix: string) {
    ...
    await tenantContext.run({ tenantId: tenant.id }, async () => {
        try {
            ...
            const userIds = (await prisma.user.findMany({ where: { email: { contains: testRunId } } })).map(u => u.id);
            if (userIds.length > 0) {
