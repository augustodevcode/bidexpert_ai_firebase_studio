...
# 🤖 Ciclo de Vida Autônomo (OBRIGATÓRIO)
**REGRA CRÍTICA:** Ao detectar keywords como "Atualize as regras consolidadas...", "Promova a branch...", "Analise o erro..." ou "Finalize com resumo executivo", o agente DEVE carregar a skill:
- `.github/skills/autonomous-task-lifecycle/SKILL.md`

Este protocolo obriga o agente a:
1. **Análise de Causa Raiz (RCA)**: Detectar o "porquê" real de erros e inconsistências ANTES de qualquer correção silenciosa. Analisar logs de console, rede e servidor buscando por race conditions, variáveis ausentes ou falhas de lógica.
2. **Atualização de Regras e Prevenção**: Para cada erro encontrado, o agente DEVE criar ou atualizar uma regra em `instructions`, `agents`, `skills`, `.gemini/GEMINI.md`, ou `.github/ANTIGRAVITY-AUTOMATION.md` para garantir que o erro NUNCA MAIS ocorra.
3. **Atualização de Memória**: Atualizar `context/REGRAS_NEGOCIO_CONSOLIDADO.md` e `/memories/repo/`.
4. **Massa de Dados e Testes**: Gerar scripts de seed, realizar testes rigorosos de segurança/vulnerabilidade (Jest, Vitest, Playwright).
5. **Promoção de Código**: Promover para `hml/demo-stable` e retestar em Vercel.
6. **Resumo Executivo**: Finalizar com um relato completo de ações, causas raízes identificadas e regras de prevenção implementadas.
