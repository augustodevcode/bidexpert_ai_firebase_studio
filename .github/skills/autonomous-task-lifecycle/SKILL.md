...
Este modo define a mentalidade do agente durante a execução de melhorias e experimentos:
...
2. **Ciclo de Iteração Noturna**: O loop de experimentos NUNCA deve parar. Não peça permissão ao humano para continuar ou se é um bom ponto de parada. O humano pode estar dormindo e espera encontrar ~100 experimentos concluídos ao acordar (assumindo ~5-10 min por ciclo).
...
1. **Detectar a Causa Raiz**: Analisar logs do console (browser), network requests e logs do servidor para identificar EXATAMENTE por que o erro ocorreu (e.g., race condition, missing env var, timeout, logic bug).
...
5. **Reteste Real**: Acessar a aplicação via Vercel URL ou `demo.localhost:9005` para garantir que o ambiente HML/Stable reflete a mudança.
