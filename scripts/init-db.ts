        ...
        for (const courtData of sampleCourtsWithRelations) {
            ...
            if (districts && districts.length > 0) {
                 for (const districtData of districts) {
                    ...
                    await seedCollection(db, 'judicialDistricts', [{...district, courtId: court.id}], `courts/${court.id}`);

                    if (branches && branches.length > 0) {
                        const branchesWithIds = branches.map(b => ({...b, districtId: district.id, courtId: court.id}));
                        await seedCollection(db, 'judicialBranches', branchesWithIds, `courts/${court.id}/judicialDistricts/${district.id}`);
                    }
                 }
            }
        }
