// scripts/seed-full-journey-fixed.ts
/**
...
async function createTenant() {
  ...
  if (result.tenant) {
    ...
    log(`Tenant criado: ${result.tenant.name} (${result.tenant.id})`);
  } else {
...
async function createAdminUser() {
  ...
  store.adminUserId = user.id;
  store.userIds.push(user.id);
  log(`Usuário admin criado: ${user.email}`);
}
...
async function createAuctioneer() {
  // Verifica se já existe um leiloeiro
  const existingAuctioneer = await prisma.auctioneer.findFirst();
  if (existingAuctioneer) {
    store.auctioneerId = existingAuctioneer.id;
    return;
  }
...
async function createSeller() {
  // Verifica se já existe um comitente
  const existingSeller = await prisma.seller.findFirst();
  if (existingSeller) {
    store.sellerId = existingSeller.id;
    return;
  }
...
async function createAuction() {
  if (!store.auctioneerId || !store.sellerId) {
    throw new Error('Leiloeiro ou comitente não encontrado');
  }
...
async function createAssets() {
  if (!store.sellerId) {
    throw new Error('Comitente não encontrado');
  }
...
async function createLots() {
  if (!store.auctionId || store.assetIds.length === 0) {
    throw new Error('Leilão ou ativos não encontrados');
  }
...
async function simulateBids() {
  if (store.lotIds.length === 0 || store.userIds.length < 2) {
    throw new Error('Lotes ou usuários não encontrados');
  }
...
async function finalizeAuction() {
  ...
  for (const lotId of store.lotIds) {
    ...
    if (lot?.bids.length) {
      ...
      // Atualiza o lote com o vencedor
      ...
      await prisma.payment.create({
        data: {
          ...
          description: `Pagamento referente ao lote ${lotId}`
        }
      });
      
      log(`Pagamento criado para o lote ${lotId} - Valor: R$ ${winningBid.amount.toFixed(2)}`);
    }
  }
