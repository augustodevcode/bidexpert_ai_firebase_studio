...
async function main() {
    ...
    try {
        ...
        const tables = [
            'bids', 'user_wins', 'process_parties', 'bens', 'lots', 'auctions', 'judicial_processes', 'sellers', 'auctioneers', 'judicial_branches', 'judicial_districts', 'courts', 'cities', 'states', 'subcategories', 'lot_categories', 'users', 'roles'
        ];
        ...
        const branchIdMap = new Map<string, number>();
        const processIdMap = new Map<string, number>();
        ...
        await seedTable(client, 'states', sampleData.sampleStates, stateIdMap, {}, dbType);
        const cityDataWithFks = sampleData.sampleCities.map(c => ({...c, stateId: stateIdMap.get(c.stateId) }));
        await seedTable(client, 'cities', cityDataWithFks, new Map(), {}, dbType);
        
        await seedTable(client, 'auctioneers', sampleData.sampleAuctioneers, auctioneerIdMap, {}, dbType);
        await seedTable(client, 'sellers', sampleData.sampleSellers, sellerIdMap, { judicialBranchId: branchIdMap }, dbType);

        await seedTable(client, 'courts', sampleData.sampleCourts, courtIdMap, {}, dbType);
        const districtDataWithFks = sampleData.sampleJudicialDistricts.map(d => ({ ...d, courtId: courtIdMap.get(d.courtId), stateId: stateIdMap.get(d.stateId) }));
        await seedTable(client, 'judicial_districts', districtDataWithFks, districtIdMap, {}, dbType);
        const branchDataWithFks = sampleData.sampleJudicialBranches.map(b => ({ ...b, districtId: districtIdMap.get(b.districtId) }));
        await seedTable(client, 'judicial_branches', branchDataWithFks, branchIdMap, {}, dbType);
        
        const processDataWithFks = sampleData.sampleJudicialProcesses.map(p => ({
            ...p,
            courtId: courtIdMap.get(p.courtId),
            districtId: districtIdMap.get(p.districtId),
            branchId: branchIdMap.get(p.branchId),
            sellerId: sellerIdMap.get(p.sellerId!)
        }));
        await seedTable(client, 'judicial_processes', processDataWithFks, processIdMap, {}, dbType);

        await seedTable(client, 'bens', sampleData.sampleBens, new Map(), { judicialProcessId: processIdMap, categoryId: categoryIdMap }, dbType);
        ...
        const auctionDataWithFks = sampleData.sampleAuctions.map(a => ({
          ...
          judicialProcessId: processIdMap.get(a.judicialProcessId!)
        }));
