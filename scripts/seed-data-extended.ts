...
const entityStore: {
  ...
  users: (Prisma.UserGetPayload<{}> & { roleNames: string[] })[];
  categories: (Prisma.LotCategoryGetPayload<{ include: { subcategories: true } }>)[];
  states: Prisma.StateGetPayload<{}>[];
  cities: Prisma.CityGetPayload<{}>[];
  courts: Prisma.CourtGetPayload<{}>[];
  judicialDistricts: Prisma.JudicialDistrictGetPayload<{}>[];
  judicialBranches: Prisma.JudicialBranchGetPayload<{}>[];
  sellers: Prisma.SellerGetPayload<{}>[];
  auctioneers: Prisma.AuctioneerGetPayload<{}>[];
  judicialProcesses: Prisma.JudicialProcessGetPayload<{}>[];
  assets: Prisma.AssetGetPayload<{}>[];
  auctions: (Prisma.AuctionGetPayload<{ include: { stages: true } }>)[];
  lots: Prisma.LotGetPayload<{}>[];
  mediaItems: Prisma.MediaItemGetPayload<{}>[];
  documentTypes: Record<string, bigint>;
  userWins: Prisma.UserWinGetPayload<{}>[];
  vehicleMakes: Prisma.VehicleMakeGetPayload<{}>[];
  vehicleModels: Prisma.VehicleModelGetPayload<{}>[];
} = {
  tenantId: 1n,
  roles: {},
  users: [],
...
const TOTAL_SELLERS = 150;
const TOTAL_AUCTIONEERS = 50;
const TOTAL_ASSETS = 3000;
const TOTAL_AUCTIONS = 750;
...
async function seedJudicialInfra() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const courtResult = await services.court.createCourt({ name: `Tribunal de Justiça de ${state.name}`, stateUf: state.uf, website: null, createdAt: new Date(), updatedAt: new Date() });
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if(state) {
            const districtResult = await services.judicialDistrict.createJudicialDistrict({
                name: `Comarca de ${faker.location.city()}`, courtId: court.id, stateId: state.id
            });
    ...
    for (let i = 0; i < 20; i++) {
        ...
        const branchResult = await services.judicialBranch.createJudicialBranch({
            name: `${i + 1}ª Vara Cível`, districtId: district.id
        });
...
async function seedParticipants() {
    ...
    for (let i = 0; i < TOTAL_USERS; i++) {
        ...
        await services.user.createUser({
            ...
            roleIds: [entityStore.roles.BIDDER, entityStore.roles.USER].map(id => id.toString()),
            tenantId: entityStore.tenantId,
        });
    }
...
async function seedAuctionsAndLots() {
    ...
    for (let i = 0; i < TOTAL_AUCTIONS; i++) {
        ...
        if (auction) {
            ...
            await seedAuctionHabilitations(auction.id);
        }
    }
    log(`${entityStore.auctions.length} leilões e ${entityStore.lots.length} lotes criados.`, 1);
}

async function seedAuctionHabilitations(auctionId: bigint) {
...
async function seedInteractions() {
    ...
    for (const lot of lotsForBidding) {
        let currentPrice = Number(lot.initialPrice);
    ...
    for (const lot of lotsWithBids) {
        if (faker.datatype.boolean(0.3)) {
            ...
            await services.userLotMaxBid.createOrUpdateUserLotMaxBid({
                ...
                lot: { connect: { id: lot.id } },
                maxAmount: Number(lot.initialPrice) + faker.number.int({ min: 500, max: 5000 }),
            });
        }
    }
}
    ...
    for (const lot of entityStore.lots) {
        if (faker.datatype.boolean(0.2)) {
            ...
            const questionResult = await services.lotQuestion.create({
                ...
                authorName: user.fullName || 'Anônimo', 
        ...
        if (faker.datatype.boolean(0.5)) {
            ...
            if (lot && user) {
                await services.review.create({
                    ...
                    authorName: user.fullName || 'Avaliador',
